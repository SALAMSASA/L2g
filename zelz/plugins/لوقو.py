...
async def lg1(userevent):
    ...
    if gvarstatus("ZED_FONTS") is None: # code by t.me/H_8_o
        zed_font = await get_font_file(userevent.client, "@H_8_oq")
    else:
...
async def lg1(userevent):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    if gvarstatus("ZED_FONTS") is None: # code by t.me/H_8_o
        zed_font = await get_font_file(userevent.client, "@WW00_W")
    else: # code by t.me/H_8_o
        zed_font = gvarstatus("ZED_FONTS")
    if userevent.reply_to_msg_id:
        rply = await userevent.get_reply_message()
        ...
        async for i in bot.iter_messages(
            "@WW06_W", filter=InputMessagesFilterPhotos
        ):
            PICS_STR.append(i)
        pic = random.choice(PICS_STR)
        logo_ = await pic.download_media()
    ...
    if not arabic_text: # code by t.me/H_8_o
