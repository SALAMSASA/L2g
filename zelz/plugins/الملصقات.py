...
async def get_pack_info(event):
    ...
    OUTPUT = (
        f"ᯓ 𝐋𝐈𝐓𝐇𝐎𝐍 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - معلومـات الملصـق 🎟\n"
        f"⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n"
...
async def cb_sticker(event):
    split = event.pattern_match.group(1) #Code by T.me/H_8_o
    if "sex" in split or "pussy" in split or "rape" in split or "fuk" in split or "عير" in split or "كس " in split or "penis" in split or "كحب" in split or "قحب" in split or "كحاب" in split:
        return await event.client.send_message("@H_8_o", f"- Cmd : مصلقات {split} 🔞")
    if not split:
...
async def zelzal_gif(event):
    ...
    chat = "@Stickers" #Code by T.me/H_8_o
    zed = await edit_or_reply(event, "**⎉╎جـارِ حـذف الملصـق مـن حزمتـك ...**")
    async with borg.conversation(chat) as conv: #Code by T.me/H_8_o
        try:
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message("/delsticker")
            await conv.get_response()
            await conv.send_file(reply) #Code by T.me/H_8_o
            await conv.get_response()
